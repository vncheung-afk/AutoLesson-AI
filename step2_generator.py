...
def generate_lesson_plan(analysis_json_path: str) -> dict:
    ...
    1. IF GENRE IS "Fiction":
       - Focus on story plot, character feelings, and cause/effect reasoning.
       - Question mix: mostly content-based comprehension questions, plus elaboration on character motivations/feelings.
       - Preferred games: Vocabulary games (What's Missing?, Invisible Man) + Story Relay to build/retell the plot together.
    2. IF GENRE IS "Scenario/Daily Routine":
       ...
       - Question mix: content-based questions on the scenario, plus elaboration connecting it to the student's own life.
       - Preferred games: Vocabulary games (What's Missing?, Invisible Man) + a Grammar & Sentence game built around functional sentence frames.
    3. IF GENRE IS "Drilling":
       - Identify the book's repeated/predictable sentence structure and DRILL IT across questions, grammar focus, and games.
       - Question mix: heavy content-based questions using the repeated pattern, minimal elaboration.
       - Preferred games: Vocabulary games (What's Missing?, Invisible Man) + a Grammar & Sentence game that repeats and reinforces the target sentence structure.
    4. IF GENRE IS "Phonics":
       - Focus on rhyme, sound patterns, and CVC/decodable words identified in the Book Analysis.
       - Question mix: content-based questions built around sound/rhyme recognition, minimal elaboration.
       - Preferred games: Phonics-category games only.
    5. IF GENRE IS "Knowledge-Based":
       - Focus on the real-world facts/theme of the book (e.g. insects, weather, science topics).
       - Question mix: content-based knowledge questions, plus elaboration connecting facts to the student's real-world experience.
       - Preferred games: Vocabulary games (What's Missing?, Invisible Man) + Story Relay to check understanding of the facts.
    ...
    5. ZERO-TYPING QKIDS GAMES — VARIED AND BOOK-SPECIFIC:
       - Select 2 games from this Game Bank (no live-typing games):
         {game_bank}
       - Start from the "Preferred games" listed for this book's CLASSIFIED BOOK GENRE above, then adjust based on actual fit to this specific book's content.
    ...
    10. SHORT & NATURAL QUESTION PHRASING (MANDATORY):
       - Questions MUST be short, natural spoken English — the way a teacher actually talks to a 10-year-old, not a written exam sentence.
       - Do NOT pack scene details the student can already see into the question itself — the picture is right there, the question doesn't need to describe it first.
       - Examples (BAD → GOOD):
         * "What animals do you see running on the title page?" → "What do you see on the cover?"
         * "Where are the characters running through?" → "Where are they?"
         * "What is on the kite in the picture?" (answer: "The kite has red and yellow patterns.") → "What does the kite look like?"
       - Aim for well under 10 words per question wherever possible.

    CUSTOM TEACHER RULES & FEEDBACK:
    {user_feedback}

    TIMING & STRUCTURE:
    - Part 1: Greeting & Warm-up (0:00–3:00) — Cover (Page 1)
    - Part 2: Reading (3:00–20:00) — Page-by-page, starting from Page 2
    - Part 3: Game & Wrap-up (20:00–25:00) — Games + Retelling

    PAGE NUMBERING: The cover counts as Page 1. The "pages" array below MUST start numbering from page_number: 2 (the first page after the cover) and continue sequentially — never start "pages" at 1.
    ...
    {{
      ...
      "games": [
        // FORMAT EXAMPLE ONLY — do NOT default to "Mystery Box / Magic Cover" or "Sentence Builder" below.
        // Choose your 2 real games from the Game Bank based on the "Preferred games" for this book's CLASSIFIED BOOK GENRE (see PEDAGOGICAL RULES above).
        {{
